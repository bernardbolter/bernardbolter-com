/**
 * Allow relation-only edition tiers — fallback columns may be null when
 * series_edition_tier_id is populated (v2 architecture).
 *
 * Usage: npx tsx src/scripts/relax-series-edition-tier-fallback-nullability.ts
 */
...
async function main() {
  ...
  await relaxNotNull(pool, 'artworks_megacities_print_editions', 'tier')
  await relaxNotNull(pool, 'artworks_megacities_print_editions', 'edition_size')
