# Vision Analysis Prompt Spec — Artwork Records

**Status: FROZEN — version A-1.0**
**Date frozen: July 11, 2026**

This is the single source of truth for the **artwork** vision analysis prompt (`visionAnalyses[]` on Artwork records). Any change to the prompt text requires a new version number (A-1.1, A-2.0, etc.) and a new entry in `vision-prompt-changelog.md` — never edit this file's frozen prompt in place.

For **FieldNotes keyframe tagging** (Moondream, comma-separated tags), see `moondream-prompt-spec.md` instead.
