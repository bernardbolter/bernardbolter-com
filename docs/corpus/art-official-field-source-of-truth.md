...
**2026-07-28** — `currentLocation.category`: added `unknown` (label "Unknown / unlocated") for confirmed-unknown whereabouts. Distinct from leaving the field blank and from `provenanceOriginKnown`. Speculative guesses still use `provenanceConfidenceLayer`.
...
**2026-07-27** — `artHistoricalReferences` RE-RESOLVED: structured-array staging permitted via JSON package / `update_field` (same agent-suggests / artist-confirms pattern as tag fields). Prose-in-relationship-field bug fix from 2026-07-23 stands — strings still structurally rejected at tool-call level. See `docs/historicalReferences/addendum-art-historical-references-staging-2026-07-27.md`.
...
| 6. Deep interpretive | `artHistoricalReferences` | relationship → ArtHistoricalReferences (hasMany) | **RESOLVED 2026-07-23** (prose-in-relationship-field bug fixed); **RE-RESOLVED 2026-07-27**: structured-array staging permitted via JSON package, same pattern as tag fields; prose still structurally rejected at tool-call level. Shape: `[{ name, matchStrategy: "fuzzy-match-or-create", relevanceNote? }]`. |
...
| 7. Where has this lived | `currentLocation.category` (select), `currentLocation.locationDetail` (text, nested) | group | confirmed | `category` enum: `artists-studio \| private-collection \| institution \| on-loan \| unknown`. **Updated 2026-07-28:** `unknown` = confirmed-unknown / unlocated (honest staging), not blank and not `provenanceOriginKnown`. Speculative location guesses still go to `provenanceConfidenceLayer` (`confidenceLevel: speculation`). Write as `currentLocation: { category, locationDetail }`. **Skip entirely** if `isOriginalTier` edition |
